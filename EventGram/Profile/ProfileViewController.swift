...
class ProfileViewController: UIViewController {
    ...
    override func viewDidLoad() {
        ...
        buttonLogout.setTitleColor(
            UIColor(
                red: 190 / 255, green: 40 / 255, blue: 60 / 255, alpha: 1.0),
            for: .normal)

        buttonLogout.addTarget(
        ...
        profileScreen.eventsTableView.register(
            TableViewEventCell.self, forCellReuseIdentifier: "eventCell")
        ...
        userRef.getDocument { [weak self] document, error in
                ...
                let userRole = data["role"] as? String
            {
                DispatchQueue.main.async {
                    self?.profileScreen.nameLabel.text =
                        data["name"] as? String ?? "Name not available"
                    self?.profileScreen.universityLabel.text =
    ...
    func fetchStudentEvents() {
        ...
        userRef.getDocument { [weak self] document, error in
            ...
            if let document = document,
                let registeredEventIds = document["registeredEvents"]
                    as? [String]
            {
    ...
    func fetchAdminEvents() {
        ...
        eventsCollection.whereField(
            "userId", isEqualTo: Auth.auth().currentUser?.uid ?? ""
        ).getDocuments { [weak self] snapshot, error in
            if let error = error {
                self?.showAlert(
                    message:
                        "Error fetching events for admin role: \(error.localizedDescription)"
                )
                return
            }
    ...
    func fetchRegisteredEvents(eventIds: [String]) {
        ...
        for eventId in eventIds {
            ...
            eventsCollection.whereField("eventId", isEqualTo: eventId)
                .getDocuments { snapshot, error in
                    if let error = error {
                        print(
                            "Error fetching event with ID \(eventId): \(error.localizedDescription)"
                        )
                        dispatchGroup.leave()
                        return
                    }
    ...
    func showAlert(message: String) {
        let alert = UIAlertController(
            title: "Profile Error", message: message, preferredStyle: .alert)
...
extension ProfileViewController: UITableViewDataSource, UITableViewDelegate {
        ...
        -> UITableViewCell
    {
        ...
        cell.labelDate.text = dateFormatter.string(
            from: registeredEvents[indexPath.row].date)

        if let imageUrl = registeredEvents[indexPath.row].imageUrl,
